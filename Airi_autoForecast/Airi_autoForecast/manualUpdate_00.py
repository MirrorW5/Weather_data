...
sv = Service('manualUpdate_00', help_='''
手动更新1
'''.strip())
...
@sv.on_prefix(('手动更新自动预报1'))
async def manualUpdate_00(bot, ev: CQEvent):
    flag_renew = 0
    time_h = int(datetime.datetime.now().strftime('%H'))
    time_m = int(datetime.datetime.now().strftime('%M'))

    if time_h == 0 and time_m >= 1 and time_m <= 55:
        flag_renew = 1
    if flag_renew == 0:
        msg = f'不在更新时段（00时01分-00时55分），请发送手动更新预报2'
    ...
    if flag_renew == 1:
        ...
        try:
            ...
            msg = f'【报错信息】自动预报00时更新出错\n原因:{e}\n\n请输入【手动更新自动预报1】'
            await bot.send(ev, msg)
            sv.logger.info('============手动更新自动预报00出错============')