...
minU_szmb = []
maxU_szmb = []
#sv = Service('autoForecast_00', help_='''
#自动预报2daysProb
#'''.strip())

#@sv.scheduled_job('cron', hour='00', minute='00',second='50')
async def autoForecast_00():
    headers = {
    ...
    try:
        url_mete_2days = 'https://meteologix.com/cn/ajax_pub/fccompact?city_id=1795565&lang=en&unit_t=celsius&unit_v=kmh&unit_l=metrisch&unit_r=joule&unit_p=hpa&nf=point&tf=0&mos_station_id=&m=rapid-euro'
        getdata_mete_2days = requests.get(url_mete_2days, headers=headers)
        info_mete_2days = getdata_mete_2days.text


        index1 = info_mete_2days.find('data_rainpop')
        info_mete_2days = info_mete_2days[index1+5:]
        index1 = info_mete_2days.find('data')
        info_mete_2days = info_mete_2days[index1:]
        index1 = info_mete_2days.find('[')
        info_mete_2days = info_mete_2days[index1:]
        index2 = info_mete_2days.find('color')
        info_mete_2days = info_mete_2days[:index2]
        ...
        while 1:
            index1 = info_mete_2days.find(',')
            index2 = info_mete_2days.find(']')
            prob_list[p] = info_mete_2days[index1+2:index2]
            info_mete_2days = info_mete_2days[index2+2:]
            p = p + 1
            if info_mete_2days.find('[') == -1:
                break

        for i in prob_list:
            prob_list[i] = round(float(prob_list[i]))


        def findMaxProb(prob_list,start,end):
            prob_max = 0
            for i in range(start,end):
                if int(prob_list[i]) > int(prob_max):
                    prob_max = prob_list[i]
            prob_max = (int(prob_max)//5)*5
            return prob_max

        prob_6h = []
        for i in range(0,8):
            prob_6h.append(str(findMaxProb(prob_list,i*6,i*6+6)))

        # print(prob_6h)

        data_list = []
        with open("C:/Users/Administrator/Desktop/myBot/HoshinoBot/hoshino/modules/Airi_autoForecast/day2_temp.txt", 'r',
                  encoding="utf-8") as f:
            data = f.read()
            data_list = data.split(',')

        str_day0 = str(data_list[0])+','+str(data_list[1])+','+str(data_list[2])
        for i in prob_6h:
            str_day0 = str_day0 + ',' + i

        with open("C:/Users/Administrator/Desktop/myBot/HoshinoBot/hoshino/modules/Airi_autoForecast/day2.txt", 'w',
                  encoding="utf-8") as f:
            f.write(str_day0)
        ...
        with open("C:/Users/Administrator/Desktop/myBot/HoshinoBot/hoshino/modules/Airi_autoForecast/add_temp.txt", "r",encoding="utf-8") as f:
            sun_temp = f.read()
        ...
        def getSzmbMessage():
            global date_szmb,temp_max_szmb,temp_min_szmb,wd_szmb,minWs_szmb,maxWs_szmb,minU_szmb,maxU_szmb
            ...
            for i in range(0,int(len(info_szmb_list))):
                ...
                wd_szmb.append(str(weather_dic['wd']))
                minWs_szmb.append(str(weather_dic['minWs']))
                maxWs_szmb.append(str(weather_dic['maxWs']))
                minU_szmb.append(str(weather_dic['minU']))
                maxU_szmb.append(str(weather_dic['maxU']))
        ...
        global wd_szmb,maxWs_szmb,minWs_szmb,maxU_szmb,minU_szmb
        ...
        with open("C:/Users/Administrator/Desktop/myBot/HoshinoBot/hoshino/modules/Airi_autoForecast/renewTime.txt", 'w', encoding="utf-8") as f:
            f.write(renewTime)
        # print(wd_szmb,maxWs_szmb,minWs_szmb)
        wind_str = wd_szmb[0]+','+maxWs_szmb[0]+','+minWs_szmb[0]+','+wd_szmb[1]+','+maxWs_szmb[1]+','+minWs_szmb[1]
        with open("C:/Users/Administrator/Desktop/myBot/HoshinoBot/hoshino/modules/Airi_autoForecast/wind.txt", 'w', encoding="utf-8") as f:
            f.write(wind_str)
        
        humidity_str = maxU_szmb[0] + ',' + minU_szmb[0] + ',' + maxU_szmb[1] + ',' + minU_szmb[1] 
        with open("C:/Users/Administrator/Desktop/myBot/HoshinoBot/hoshino/modules/Airi_autoForecast/humidity.txt", 'w', encoding="utf-8") as f:
            f.write(humidity_str)
            
        bot = nonebot.get_bot()
        await bot.send_private_msg(user_id=2864309367, message=f'成功更新00时自动预报') 
        
    except Exception as e:
        bot = nonebot.get_bot()
        await bot.send_private_msg(user_id=2864309367, message=f'【报错信息】自动预报00时更新出错\n原因:{e}\n\n请输入【手动更新自动预报1】')
        #sv.logger.info('============自动预报00时更新出错============')
