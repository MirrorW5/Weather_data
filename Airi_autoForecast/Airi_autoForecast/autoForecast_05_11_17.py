...
#sv = Service('autoForecast_05_11_17', help_='''
#自动预报
#'''.strip())
...
#@sv.scheduled_job('cron', hour='5,11,17', minute='10',second='37')
async def autoForecast_05_11_17():
    # time_h = int(datetime.datetime.now().strftime('%H'))
    # if time_h == 11 or time_h == 5 or time_h == 17:
    headers = {
    ...
    try:
        ...
        bot = nonebot.get_bot()
        await bot.send_private_msg(user_id=2864309367, message=f'成功更新05-11-17时自动预报') 
        
    except Exception as e:
        bot = nonebot.get_bot()
        await bot.send_private_msg(user_id=2864309367, message=f'【报错信息】自动预报05-11-17时更新出错\n原因:{e}\n\n请输入【手动更新自动预报2】')
